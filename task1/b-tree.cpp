...
typename std::deque<Key>::iterator KeyBinSearchInNode(const Key& key, std::deque<Key>& scope) {
    return std::lower_bound(scope.begin(), scope.end(), key, [](const Key& x, const Key& y){ return std::less<Key>{}(x, y); });
}
...
template <typename Key, typename Value, size_t N>
typename std::deque<Value>::iterator GetValueIter(const typename std::deque<Key>::iterator& iter, std::shared_ptr<Node<Key, Value, N>>& node) {
...
template <typename Key, typename Value, size_t N>
typename std::deque<std::shared_ptr<Node<Key, Value, N>>>::iterator GetChildIter(const typename std::deque<Key>::iterator& iter, std::shared_ptr<Node<Key, Value, N>>& node) {
    const auto delta = std::distance(node->keys->begin(), iter);
...
// dive to leaf by key
template <typename Key, typename Value, size_t N>
std::shared_ptr<Node<Key, Value, N>> GoToLeaf(const Key& key, const std::shared_ptr<Node<Key, Value, N>>& node) {
    auto cur_node = node;
...
// dive to leaf by key
template <typename Key, typename Value, size_t N>
std::shared_ptr<Node<Key, Value, N>> GoToLeafAndRemember(const Key& key, const std::shared_ptr<Node<Key, Value, N>>& node, std::stack<std::shared_ptr<Node<Key, Value, N>>>& stck) {
    auto cur_node = node;
    while (!IsLeaf(cur_node)) {
...
template <typename Key, typename Value, size_t N>
Value* Search(const Key& key, const std::shared_ptr<Node<Key, Value, N>>& node) {
    if (node == nullptr) {
        return nullptr;
    }
    // go to leaf
...
// calls in insertion
template <typename Key, typename Value, size_t N>
std::shared_ptr<Node<Key, Value, N>> MoveRight(const Key& key, std::shared_ptr<Node<Key, Value, N>>& node) {
    auto cur_node = node;
...
template <typename Key, typename Value, size_t N>
std::shared_ptr<Node<Key, Value, N>> GetNodeForInsertion(const Key& key, std::shared_ptr<Node<Key, Value, N>>& node, std::stack<std::shared_ptr<Node<Key, Value, N>>>& stck) {
...
template <typename Key, typename Value, size_t N>
std::shared_ptr<Node<Key, Value, N>> SplitBigNode(std::shared_ptr<Node<Key, Value, N>>& node) {
    ...
    if (IsLeaf(node)) {
        ...
        new_node->children = std::move(std::make_unique<std::deque<std::shared_ptr<Node<Key, Value, N>>>>());
        for (size_t i = 0; i < N; ++i) {
...
template <typename Key, typename Value, size_t N>
void PutNewNodeIntoParent(size_t cur_node_idx, std::shared_ptr<Node<Key, Value, N>>& parent, const bool is_max_key) {
    if (is_max_key) {
        --cur_node_idx;
    }
...
template <typename Key, typename Value, size_t N>
std::shared_ptr<Node<Key, Value, N>> MakeRoot(std::shared_ptr<Node<Key, Value, N>>& left, std::shared_ptr<Node<Key, Value, N>>& right) {
    ...
    parent->children = std::move(std::make_unique<std::deque<std::shared_ptr<Node<Key, Value, N>>>>());
...
template <typename Key, typename Value, size_t N>
void RefreshParentsAfterInsertionOfMax(const Key& key, std::stack<std::shared_ptr<Node<Key, Value, N>>>& stck) {
    if (stck.empty()) {
        return;
    }
    while (stck.size() > 1) {
...
template <typename Key, typename Value, size_t N>
void DoInsertionIntoNode(size_t cur_node_idx, std::shared_ptr<Node<Key, Value, N>>& parent, std::shared_ptr<Node<Key, Value, N>>& root, std::stack<std::shared_ptr<Node<Key, Value, N>>>& stck, const bool is_max_key) {
...
template <typename Key, typename Value, size_t N>
void DoInsertion(std::pair<Key, Value>& item, const typename std::deque<Key>::iterator& iter, std::shared_ptr<Node<Key, Value, N>>& node, std::shared_ptr<Node<Key, Value, N>>& root, std::stack<std::shared_ptr<Node<Key, Value, N>>>& stck) {
...
template <typename Key, typename Value, size_t N>
void EraseFromSaveLeaf(const typename std::deque<Key>::iterator& iter, std::shared_ptr<Node<Key, Value, N>>& leaf) {
...
template <typename Key, typename Value, size_t N>
void RefreshParentsAfterTransfer(std::shared_ptr<Node<Key, Value, N>>& left, std::vector<std::shared_ptr<Node<Key, Value, N>>>& parents) {
...
template <typename Key, typename Value, size_t N>
void TransferL2R(std::shared_ptr<Node<Key, Value, N>>& left, std::shared_ptr<Node<Key, Value, N>>& right, std::vector<std::shared_ptr<Node<Key, Value, N>>>& parents) {
    const auto delta = ((left->keys->size() + right->keys->size() + 1) / 2) - right->keys->size();
...
template <typename Key, typename Value, size_t N>
void TransferR2L(std::shared_ptr<Node<Key, Value, N>>& left, std::shared_ptr<Node<Key, Value, N>>& right, std::vector<std::shared_ptr<Node<Key, Value, N>>>& parents) {
    const auto delta = ((left->keys->size() + right->keys->size() + 1) / 2) - left->keys->size();
    // transfer node values to neighbour
    for (size_t i = 0; i < delta; ++i) {
...
template <typename Key, typename Value, size_t N>
void RefreshParentsAfterMerge(std::shared_ptr<Node<Key, Value, N>>& left, std::vector<std::shared_ptr<Node<Key, Value, N>>>& parents) {
...
template <typename Key, typename Value, size_t N>
void MergeLRNodes(std::shared_ptr<Node<Key, Value, N>>& left, std::shared_ptr<Node<Key, Value, N>>& right, std::vector<std::shared_ptr<Node<Key, Value, N>>>& parents) {
...
template <typename Key, typename Value, size_t N>
std::shared_ptr<Node<Key, Value, N>> MakeSave(const Key& key, std::shared_ptr<Node<Key, Value, N>>& node) {
    auto cur_node = node;
    ...
    while (!IsLeaf(cur_node)) {
        ...
        if (cur_parent->keys->size() == 1) {
            while (cur_parent->children != nullptr && cur_parent->children->front()->keys->size() == 1 && !IsLeaf(cur_parent->children->front())) {
                cur_parent->children->front() = std::move(cur_parent->children->front()->children->front());
            }
        ...
        if (idx != 0) {
            ...
            if (IsSaveNodeForDeletion(left)) {
                ...
                left->mtx.unlock();
            } else if (idx < cur_parent->children->size() - 1 && IsSaveNodeForDeletion((*(cur_parent->children))[idx + 1])) {
...
template <typename Key, typename Value, size_t N>
void MakeSaveAndErase(const Key& key, std::shared_ptr<Node<Key, Value, N>>& root) {
    root->mtx.lock();
...
void BLinkTree<Key, Value, N>::emplace(Args&&... args) {
    auto item = std::make_unique<std::pair<Key, Value>>(std::forward<Args>(args)...);
    if (root_ == nullptr) {
